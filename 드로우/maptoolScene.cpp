...
HRESULT maptoolScene::init()
{
	...
	_temp = false;			//일시적으로 클릭위치저장하기위한 불함수
	return S_OK;
}
...
void maptoolScene::update()
{
	...
	if (INPUT->GetKey(VK_RBUTTON))
	{
		...
		{
			//처음 클릭한 곳의 좌표를 기억하고 마우스가 이동한 만큼의 렉트를 생성하는 부분
			if (PtInRect(&_tiles[i].rc, _ptMouse))
			{
				...
				{
					_rcMulti = RectMake(_tempLeft, _tempTop, _tiles[i].rc.right - _tempLeft, _tiles[i].rc.bottom - _tempTop);
				}
				else if(_tempLeft > _tiles[i].rc.left&&_tempTop>_tiles[i].rc.top)
				{
					_rcMulti = RectMake(_tiles[i].rc.left, _tiles[i].rc.top, _tempRight-_tiles[i].rc.left, _tempBottom-_tiles[i].rc.top);
				}
				else if (_tempLeft < _tiles[i].rc.left&&_tempTop > _tiles[i].rc.top)
				{
					_rcMulti = RectMake(_tempLeft, _tiles[i].rc.top, _tiles[i].rc.right - _tempLeft, _tempBottom - _tiles[i].rc.top);
				}
				else if (_tempLeft > _tiles[i].rc.left&&_tempTop < _tiles[i].rc.top)
				{
					_rcMulti = RectMake(_tiles[i].rc.left, _tempTop, _tempRight - _tiles[i].rc.left, _tiles[i].rc.bottom- _tempTop);
				}
	...
	//A버튼을 누를 시 골라진 타일을 선택된 타일로 전부 바꾼다
	if (INPUT->GetKeyDown('A'))
	{
		for (int i = 0; i < TILEX * TILEY; i++)
		{
			if (PtInRect(&_tiles[i].rc, _ptMouse))
			{
				if (_ctrlSelect == CTRL_TERRAIN)
				{
					_SelectTilesX = _tiles[i].terrainFrameX;
					_SelectTilesY = _tiles[i].terrainFrameY;
				}
				if (_ctrlSelect == CTRL_OBJECT)
				{
			...
			for (int j = 0; j < TILEX * TILEY; j++)
			{
				...
				//현재 클릭한거의 값이 바뀌는 바람에 같이 다바껴버린다............. 그럼 판단할 만한게 무가있을까..
				if (_SelectTilesX == _tiles[j].terrainFrameX &&_SelectTilesY == _tiles[j].terrainFrameY ||
					_SelectTilesX == _tiles[j].objFrameX &&_SelectTilesY == _tiles[j].objFrameY)
				{
					if (_ctrlSelect == CTRL_TERRAIN)
					{
						_tiles[j].terrainFrameX = _currentTile.x;
						_tiles[j].terrainFrameY = _currentTile.y;
						_tiles[j].terrain = terrainSelect(_currentTile.x, _currentTile.y);
					}
					//현재버튼이 오브젝트냐?
					if (_ctrlSelect == CTRL_OBJECT)
					{
						...
						_tiles[j].obj = objectSelect(_currentTile.x, _currentTile.y);
					}
					//현재버튼이 지우개냐?
...
void maptoolScene::render()
{
	...
	//수치값 확인용
	//CHAR str[128];
	//sprintf(str, "%d %d", _tempLeft, _tempTop);
